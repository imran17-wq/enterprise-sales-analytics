...
data_cleaning.py
================
Full data-cleaning pipeline using Pandas:
  1. Standardise column names
  2. Handle missing values
  3. Remove duplicates
  4. Convert date columns
  5. Correct inconsistent text values
  6. Detect and handle outliers (IQR method)
...
def handle_missing_values(df: pd.DataFrame) -> pd.DataFrame:
    """
    Fill or drop missing values using Pandas strategies:
...
def handle_outliers_iqr(df: pd.DataFrame,
    ...
    ----------
    df     : DataFrame
    cols   : list of column names to check (defaults to NUMERIC_OUTLIER_COLS)
    method : 'cap'  → Winsorise to [Q1 - 1.5*IQR, Q3 + 1.5*IQR]
...
def clean_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Parameters
    ----------
    df : pd.DataFrame  – raw data from data_loader
