...
def build_pdf_export(df: pd.DataFrame, kpis: dict, filters: dict,
    ...
    if date_str:
        pdf.cell(0, 6, safe_text(f"Date Range: {date_str}"), align="C", new_x="LMARGIN", new_y="NEXT")
    pdf.set_y(70)
    pdf.set_text_color(*DARK)
    ...
    kpi_rows = [
        ...
        ("Best Region",      safe_text(str(kpis.get("best_region", "N/A")))),
        ("Best Product",     safe_text(str(kpis.get("best_product", "N/A")))),
    ...
    for k, v in filters.items():
        ...
        pdf.cell(0, 6, safe_text(f"{label}: {vals}"), new_x="LMARGIN", new_y="NEXT")
    pdf.set_text_color(*DARK)
    ...
    prod_rows = [
        (safe_text(r["product"]), fmt_inr(r["Revenue"]), f"{int(r['Orders']):,}", f"{r['ReturnRate']*100:.1f}%")
    ...
    reg_rows = [
        (safe_text(r["region"]), fmt_inr(r["Revenue"]), f"{int(r['Orders']):,}", f"{r['ReturnRate']*100:.1f}%")
    ...
    pdf.set_text_color(*DARK)
    pdf.multi_cell(0, 6, safe_text(summary.get("headline", "")), new_x="LMARGIN", new_y="NEXT")
    pdf.ln(3)
    ...
    for heading, key in [("Key Drivers", "drivers"), ("Risks", "risks"), ("Recommended Actions", "actions")]:
        ...
        for item in summary.get(key, []):
            ...
            pdf.multi_cell(0, 5.5, safe_text(f"  {bullet}{item}"), new_x="LMARGIN", new_y="NEXT")
        pdf.ln(3)
